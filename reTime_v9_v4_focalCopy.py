...
def duplicate_temp(sele):
    ...
    cmds.connectAttr("%s.focalLength" %sele_shape[0], "%s.focalLength" % dupli_test_shape[0])
...
def retime(ret_frames, StartTime):
    ...
    for i in ret_frames:
        ...
        copy_focal = cmds.copyKey('%s.focalLength' %dupli_test_shape[0], time= (frames,frames))
        if copy_focal:  
